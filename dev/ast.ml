...
type expr =
  ...
  | LetTup of string list * expr * expr
...
type ctype = CAny | CInt | CBool | CTuple of ctype list
...
let rec string_of_expr (e : expr) : string =
  ...
  | Prim1 (op, e) ->
      sprintf "(%s %s)"
        (match op with Add1 -> "add1" | Sub1 -> "sub1" | Print -> "print" | Not -> "not")
        (string_of_expr e)
      (* remove the print case when providing the sys interface *)
  | Prim2 (op, e1, e2) ->
      sprintf "(%s %s %s)"
        (match op with Add -> "+" | And -> "and" | Lte -> "<=" | Get -> "get" | Eq -> "=")
        (string_of_expr e1) (string_of_expr e2)
  | Let (x, e1, e2) ->
      sprintf "(let (%s %s) %s)" x (string_of_expr e1) (string_of_expr e2)
  | LetTup (x, e1, e2) ->
      sprintf "(let ((tup %s) %s) %s)" (String.concat " " x) (string_of_expr e1)
        (string_of_expr e2)
  | If (e1, e2, e3) ->
      sprintf "(if %s %s %s)" (string_of_expr e1) (string_of_expr e2)
        (string_of_expr e3)
  | Apply (fe, ael) ->
      sprintf "(%s %s)" fe (String.concat " " (List.map string_of_expr ael))
  | Tuple exprs -> sprintf "(tup %s)" (string_of_exprs exprs)
  | Set (e, k, v) ->
      sprintf "(set %s %s %s)" (string_of_expr e) (string_of_expr k)
        (string_of_expr v)
  | Lambda (params, body) ->
      sprintf "(lambda (%s) %s)" (String.concat " " params)
        (string_of_expr body)
  | LamApply (fe, ael) ->
      sprintf "(%s %s)" (string_of_expr fe)
...
and string_of_exprs (e : expr list) : string =
  match e with [] -> "" | h :: t -> " " ^ string_of_expr h ^ string_of_exprs t
...
let rec string_of_ctype (t : ctype) : string =
  ...
  | CBool -> "bool"
  | CTuple types ->
      let rec string_of_types ls =
        match ls with [] -> "" | e :: l -> e ^ "," ^ string_of_types l
      in
